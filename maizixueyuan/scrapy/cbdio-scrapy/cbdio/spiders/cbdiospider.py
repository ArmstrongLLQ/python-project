...
class CbdiospiderSpider(scrapy.Spider):
    name = "cbdiospider"
    allowed_domains = ["cbdio.com"]
    start_urls = ['http://www.cbdio.com/']
    ...
    def parse(self, response):
        ...
        titles = response.xpath('/html/body/div[4]/div[2]/div[1]/div[3]/ul/li/div/p[1]/a/text()').extract()
        times = response.xpath('/html/body/div[4]/div[2]/div[1]/div[3]/ul/li/div/p[2]/text()').extract()
        descriptions = response.xpath('/html/body/div[4]/div[2]/div[1]/div[3]/ul/li/div/p[3]/text()').extract()
        
        
        #从一级页面获取二级页面的url，存入列表second_url  
        content_urls = response.xpath('/html/body/div[4]/div[2]/div[1]/div[3]/ul/li/div/p[1]/a/@href').extract()
        second_urls = []
