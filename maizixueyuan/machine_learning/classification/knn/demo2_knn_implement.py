...
# 加载数据，从数据库文件中加载数据，split为分割参数，将数据分割为训练集和测试集
def loadDataset(filename, split, trainingSet=[], testSet=[]):
    with open(filename, 'rb') as csvfile:   # csv 用逗号为分隔符的一种文件类型
...
def main():
    ...
    for x in range(len(testSet)):
        ...
        print('> predicteclusterd=' + repr(result) + ', actual=' + repr(testSet[x][-1]))
