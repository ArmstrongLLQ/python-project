...
def updateData(excel_file):
	...
	if(index_filename == 0 or index_filesize == 0 or index_issue_date==0):
		print(excel_file)
	else:
		...
		for i in range(1, nrows):
			new_filepath = '/AIAA-FULLTXT/AIAA2018004/PDF/' + table_list[i-1]['filename'].replace('\\', '/')
...
def changeDocidById():
	...
	try:
		...
		for r in results:
			...
			sql2 = 'update s_data_1003 set docid = %d where id = %d' % (docid, id)
			# print(sql2)
			try:
				cursor.execute(sql2)
				db.commit()
...
def main():
	excel_file = 'Z:/ALLPDF/AIAA-FULLTXT/AIAA2018004/AIAA期刊2.xlsx'
	actual_filepath = 'Z:/ALLPDF/AIAA-FULLTXT/AIAA2018004/PDF/'
	table_list = excelTableByIndex(excel_file)
	...
	new_excel_file = 'Z:/ALLPDF/AIAA-FULLTXT/AIAA2018004/AIAA期刊2.new.xls'
