...
class DOAJ:
    ...
    # 将数据库插入语句封装成函数
    def insert_mysql(self, each, db, cursor):
        # sql语句
    ...
    # 进行数据爬取，并存入数据库
    def test_get_doaj_list(self, my_host, my_username, my_keyword, my_database):
        # 打开数据库连接
        ...
        if rsp.ok:
            for each in rsp.json()['results']:
                #print(each['id'])
                self.insert_mysql(each, db, cursor)
...
if __name__ == '__main__':
    ...
    test1.test_get_doaj_list("localhost", "root", "", "lilanqing")
